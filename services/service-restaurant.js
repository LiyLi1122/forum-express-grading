  ...
  getRestaurants: (req, callback) => {
    ...
    return Promise.all([
      ...
      .then(([restaurants, categories]) => { // 做縮字整理至 50 字
        ...
        return callback(null, { // callback(error, data) -> 裡面 error 判定不存在，引數 {} 所以會走 res.json(data = {})
          restaurants: data,
          categories,
          categoryId,
      ...
      .catch(error => callback(error)) // callback(error, data) -> 裡面 error 判定存在，所以會走 next(error)
  }
}
